"""
Original Source: Self-Driving Car Project Q&A & lesson  |
...
class Tracker:
    ...
    def __init__(self, mywindow_width, mywindow_height, mymargin, my_ym=1,
        ...
        # list that stores all the past (left, right) center set values used
        # for smoothing the output
        self.recent_centers = []

        # the window pixel width of the center values, used to count pixels
        # inside center windows to determine curve
        # values
        self.window_width = mywindow_width

        # the window pixel height of the center values, used to count pixels
        # inside center windows to determine curve values. breaks the image
        # into vertical levels
        self.window_height = mywindow_height

        # The pixel distance in both directions to slide (left_window +
        # right_window) template for searching
    ...
    def find_window_centroids(self, warped):
        ...
        # First find the tow starting positions for the left and right lane by
        ...
        for level in range(1, int(warped.shape[0]/window_height)):
            ...
            image_layer = \
                np.sum(warped[int(warped.shape[0]-(level+1)*window_height):
                       int(warped.shape[0]-level*window_height), :], axis=0)
            conv_signal = np.convolve(window, image_layer)
            # Find the best left centroid by using past left center as a
            # reference
